...
public class RoleJpaRepositoryAdapter implements RoleRepository {
    ...
    @Override
    public Page<Role> findFiltered(String name, Collection<String> permissionsName, Pageable pageable) {
        Specification<RoleEntity> spec = RoleSpecification.hasName(name)
                .and(RoleSpecification.hasPermissionsName(permissionsName));
    ...
    public Collection<Role> findAllByIds(Collection<UUID> uuids) {
        List<RoleEntity> roleEntities = rolesJpaRepository.findAllById(uuids);
